...
function hasloo_nav($args = array()) {
	...
	$defaults = apply_filters('hasloo_nav_default_args', $defaults);
	$args = wp_parse_args($args, $defaults);
	
	// Allow child theme to short-circuit this function
	$pre = apply_filters('hasloo_pre_nav', false, $args);
	if ( $pre ) return $pre;
	...
	if ( isset($args['show_home']) && ! empty($args['show_home']) ) {
		if ( true === $args['show_home'] || '1' === $args['show_home'] || 1 === $args['show_home'] )
			$text = apply_filters('hasloo_nav_home_text', __('Home', 'hasloo'), $args);
		else
			$text = $args['show_home'];
		...
		$menu .= hasloo_get_seo_option('nofollow_home_link') ? hasloo_rel_nofollow($home) : $home;
	...
	$menu = apply_filters('hasloo_nav_items', $menu, $args);
	...
	$menu = apply_filters('hasloo_nav', $menu, $args);
