 ...
 * @since 0.1
 */
function hasloo_get_image_id($num = 0) {
	global $post;
...
function hasloo_get_image($args = array()) {
	global $post;
	...
	$defaults = apply_filters('hasloo_get_image_default_args', $defaults);
	
	$args = wp_parse_args($args, $defaults);
	
	// Allow child theme to short-circuit this function
	$pre = apply_filters('hasloo_pre_get_image', false, $args, $post);
	...
	// else pull the first image attachment 
		...
		$id = hasloo_get_image_id($args['num']); 
		$html = wp_get_attachment_image($id, $args['size'], false, $args['attr']); 
		list($url) = wp_get_attachment_image_src($id, $args['size'], false, $args['attr']); 
	}
	...
	return apply_filters('hasloo_get_image', $output, $args, $id, $html, $url, $src);
}
/**
 ...
 * @since 0.1
 */
function hasloo_image($args = array()) {
	$image = hasloo_get_image($args);
 ...
 * @since 0.1.8
 */
add_filter('wp_get_attachment_image_attributes', 'hasloo_filter_attachment_image_attributes', 10, 2);
function hasloo_filter_attachment_image_attributes($attr, $attachment) {
 ...
 * @return array
 */
function hasloo_get_additional_image_sizes() {
 ...
 * @return array
 */
function hasloo_get_image_sizes() {
	$builtin_sizes = array(
		'large'		=> array(
	...
	$additional_sizes = hasloo_get_additional_image_sizes();
