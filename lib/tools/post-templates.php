...
function pt_add_custom_box() {
	if(get_post_templates() && function_exists( 'add_meta_box' )) {
		add_meta_box( 'pt_post_templates', __( 'Single Post Template', 'hasloo' ), 
...
function pt_inner_custom_box() {
	...
	echo '<label class="hidden" for="post_template">' . __("Post Template", 'hasloo' ) . '</label><br />';
	echo '<select name="_wp_post_template" id="post_template" class="dropdown">';
	echo '<option value="">Default</option>';
	post_templates_dropdown(); //get the options
	echo '</select><br /><br />';
	echo '<p>' . __('Some themes have custom templates you can use for single posts that might have additional features or custom layouts. If so, you will see them above.', 'hasloo' ) . '</p><br />';
}}
